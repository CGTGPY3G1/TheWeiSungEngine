...
struct AttackerInfo {
	AttackerInfo(std::weak_ptr<GameObject> attacker = std::weak_ptr<GameObject>(), const std::string & attackerName = "", const unsigned int & attackerID = 0);
	...
	friend bool operator == (const unsigned int & lhs, const AttackerInfo & rhs);
	std::string name;
...
public:
	...
	void SetAttacker(std::weak_ptr<GameObject> attacker = std::weak_ptr<GameObject>(), const std::string & name = "");
