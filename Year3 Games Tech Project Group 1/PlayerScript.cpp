	...
	if(!driving) {
		...
		if(input->GetKeyDown(KeyCodes::KeyCode::E) || input->GetKeyDown(KeyCodes::KeyCode::Space) || input->GetControllerButtonDown(0, ControllerButtons::ControllerButton::Y)) {
			...
			if(c) {
				...
				std::shared_ptr<GameObject> p = player.lock();
				if(p) {
					p->GetComponent<RigidBody2D>().lock()->SetPosition(carTransform->GetPosition() + (carTransform->GetRight() * Physics::PIXELS_PER_METRE) + (carTransform->GetForward() * Physics::PIXELS_PER_METRE));
					p->GetComponent<RigidBody2D>().lock()->SetEnabled(true);
...
int PlayerScript::GetSortOrder() {
	return TypeInfo::ScriptSortOrder<PlayerScript>();
}
