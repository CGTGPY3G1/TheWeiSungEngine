...
namespace WeiSungEngine {
	...
	struct KillerData {
		friend struct std::hash<KillerData>;
...
namespace std {
	template <>
	struct hash<WeiSungEngine::KillerData> {
		std::size_t operator()(const WeiSungEngine::KillerData& data) const {
			return ((hash<string>()(data.killerName) ^ (hash<unsigned int>()(data.killerId) << 1)) >> 1);
		}
	};
}
namespace WeiSungEngine {
